...
namespace Snyk.Fixes.Common.Exceptions
{
    ...
    public class DomainRuleViolationException : ApplicationExceptionBase
    {
        ...
        public static void Raise(OperationCode operationCode, Guid correlationId, Guid localhostId, DomainExceptionBase domainException)
        {
            var exception = new DomainRuleViolationException(
                (ExceptionCode)domainException.ExceptionCode,
                operationCode,
                correlationId,
                $"A domain exception has been raised in the localhost '{localhostId}'. {domainException.Message}",
                domainException
            );
        ...
        public static void Raise(IOperationContext context, Guid localhostId, DomainExceptionBase domainException)
        {
            Raise((OperationCode)context.OperationCode, context.CorrelationId, localhostId, domainException);
        }
    }
}

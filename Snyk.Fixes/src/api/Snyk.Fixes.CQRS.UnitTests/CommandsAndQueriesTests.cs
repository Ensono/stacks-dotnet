...
namespace Snyk.Fixes.CQRS.UnitTests
{
    /// <summary>
    ...
    public class CommandsAndQueriesTests
    {
        ...
        {
            var assembly = typeof(Createlocalhost).Assembly;
        ...
        {
            var definitions = typeof(Createlocalhost).Assembly.GetImplementationsOf(typeof(ICommand));
        ...
        {
            var definitions = typeof(Createlocalhost).Assembly.GetImplementationsOf(typeof(IOperationContext));
        ...
        {
            var definitions = typeof(GetlocalhostById).Assembly.GetImplementationsOf(typeof(IQueryCriteria));
            foreach (var definition in definitions)
            {
                var operationCode = GetOperationCode(definition.implementation);
                var operationName = GetOperationName(operationCode);
        ...
        {
            var commands = typeof(Createlocalhost)
                .Assembly
        ...
        {
            var queries = typeof(GetlocalhostById)
                .Assembly
                .GetImplementationsOf(typeof(IQueryCriteria))
                .Select(c => c.Item2);

            var handlers = typeof(GetlocalhostByIdQueryHandler)
                .Assembly

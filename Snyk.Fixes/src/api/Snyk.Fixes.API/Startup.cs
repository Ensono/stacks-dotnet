...
using Snyk.Fixes.API.Authorization;
using Snyk.Fixes.API.Models.Requests;

namespace Snyk.Fixes.API
{
    public class Startup
    {
            ...
            IOptions<JwtBearerAuthenticationConfiguration> jwtBearerAuthenticationOptions)
        {
            ...
            .UseSwaggerUI(c =>
            {
                ...
                c.SwaggerEndpoint("all/swagger.json", "localhost (all)");
                c.SwaggerEndpoint("v1/swagger.json", "localhost (version 1)");
                c.SwaggerEndpoint("v2/swagger.json", "localhost (version 2)");
            ...
            services
                ...
                .AddSwaggerGen(c =>
                {
                    c.SwaggerDoc("all", new OpenApiInfo
                    {
                        Version = "all",
                        Title = "localhost API",
                        Description = "APIs used to interact and manage localhosts for a restaurant",
                    ...
                    c.IncludeXmlComments($"{AppContext.BaseDirectory}{Path.DirectorySeparatorChar}{typeof(CreatelocalhostRequest).Assembly.GetName().Name}.xml");

                    //Set default tags, shows on top, non defined tags appears at bottom
                    c.DocumentFilter<SwaggerDocumentTagger>(new OpenApiTag[] {
                        new OpenApiTag { Name = "localhost" },

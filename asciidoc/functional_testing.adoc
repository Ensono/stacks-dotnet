...
Example: In `appsettings.json`, we are using the configuration setting (key-value pair) "BaseUrl":"link:http://dev.azure.ensonostacks.com/api/menu/[]". If there is an Environment Variable set on the current machine/build agent using `BaseUrl` key, the value in `appsettings.json` will be replaced.
